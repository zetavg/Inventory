...
- (void)receiveDataWithBLEDataSource:(NSMutableArray *)dataSourceEPC
                       dataSourceTID:(NSMutableArray *)dataSourceTID
                      dataSourceUSER:(NSMutableArray *)dataSourceUSER
                                RSSI:(NSInteger)RSSI;
...
@property(nonatomic, copy) NSString *
    tagTypeStr; // 判斷連續獲取新標籤的時候返回的類型是epc還是epc+tid還是epc+tid+user
...
- (void)setRegionWithsaveStr:(NSString *)saveStr
                   regionStr:(NSString *)regionStr; // 區域設置
...
// password:4個字節的訪問密碼.  MMBstr:掩碼的數據區(0x00爲Reserve
// 0x01爲EPC，0x02表示TID，0x03表示USR). MSAstr:爲掩碼的地址。
// MDLstr:爲掩碼的長度。 Mdata:爲掩碼數據。 MBstr:爲要寫的數據區(0x00爲Reserve
// 0x01爲EPC，0x02表示TID，0x03表示USR)  SAstr :爲要寫數據區的地址。 DLstr
// :爲要寫的數據長度(字爲單位)。 isfilter表示是否過濾
- (void)readLabelMessageWithPassword:(NSString *)password
                            ...
                            isfilter:(BOOL)isfilter; // 讀標籤數據區   成功

// password:4個字節的訪問密碼.  MMBstr:掩碼的數據區(0x00爲Reserve
// 0x01爲EPC，0x02表示TID，0x03表示USR). MSAstr:爲掩碼的地址。
// MDLstr:爲掩碼的長度。 Mdata:爲掩碼數據。 MBstr:爲要寫的數據區(0x00爲Reserve
// 0x01爲EPC，0x02表示TID，0x03表示USR)  SAstr :爲要寫數據區的地址。 DLstr
// :爲要寫的數據長度(字爲單位)。 writeData :爲寫入的數據，高位在前。
// isfilter表示是否過濾
- (void)writeLabelMessageWithPassword:(NSString *)password
                            ...
                            writeData:(NSString *)writeData
                             isfilter:(BOOL)isfilter; // 寫標籤數據區   成功

// AP 爲標籤的 AccPwd 值;MMB 爲啓動過濾操作的 bank 號，0x01 表 示 EPC，0x02 表示
// TID，0x03 表示 USR，其他值爲非法值;MSA 爲啓動過濾 操作的起始地址，單位爲
// bit;MDL爲啓動過濾操作的過濾數據長度，單位爲 bit，0x00 表示無過濾;MData
// 爲啓動過濾時的數據，單位爲字節，若 MDL 不足整數 倍字節，不足位低位補 0;LD 共
// 3 個字節 24bit，其中，高 4bit 無效，第 0~9bit(共10bit)爲 Action 位，第
// 10~19bit(共 10bit)爲 mask 位 isfilter表示是否過濾
- (void)lockLabelWithPassword:(NSString *)password
                     ...
                     isfilter:(BOOL)isfilter; // Lock標籤

// KP 爲標籤的 KillPwd 值;MMB 爲啓動過濾操作的 bank 號，0x01 表 示 EPC，0x02
// 表示 TID，0x03 表示 USR，其他值爲非法值;MSA 爲啓動過濾 操作的起始地址，單位爲
// bit;MDL爲啓動過濾操作的過濾數據長度，單位爲 bit， 0x00 表示無過濾;MData
// 爲啓動過濾時的數據，單位爲字節，若 MDL 不足整數 倍字節，不足位低位補
// 0;當標籤的 KillPwd 區的值爲 0x00000000 時，標籤會忽 略 kill 命令，kill
// 命令不會成功 isfilter表示是否過濾
- (void)killLabelWithPassword:(NSString *)password
                       MMBstr:(NSString *)MMBstr
                       MSAstr:(NSString *)MSAstr
                       MDLstr:(NSString *)MDLstr
                       MDdata:(NSString *)MDdata
                     isfilter:(BOOL)isfilter; // kill標籤

- (void)getLabMessage; // 獲取標籤數據 可以
- (void)setSM4PassWordWithmodel:(NSString *)model
                       password:(NSString *)password
                     originPass:(NSString *)originPass; // 設置密鑰   可以
...
- (void)encryptionUSERWithaddress:(NSString *)address
                        lengthStr:(NSString *)lengthStr
                          dataStr:(NSString *)dataStr; // USER加密  可以
- (void)decryptUSERWithaddress:(NSString *)address
                     lengthStr:(NSString *)lengthStr; // USER解密 可以
...
//- (NSData *)setFilterSendDataWithUfBank:(char)ufBank ufPtr:(int)ufPtr
// dataLen:(int)datalen hexDataBuf:(NSString *)hexDatabuf;
