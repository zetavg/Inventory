...
@property(nonatomic, strong)
    NSTimer *connectTime; // 計算藍牙連接是否超時的定時器
@property(nonatomic, strong)
    NSTimer *sendGetTagRequestTime; // 定時發送獲取標籤命令
...
- (void)bleDoScan {
  self.bleScanTimer =
      [NSTimer scheduledTimerWithTimeInterval:1
                                       target:self
                                     selector:@selector(startBleScan)
                                     userInfo:nil
                                      repeats:YES];
}
...
- (BOOL)connectPeripheralWithIdentifier:(NSString *)identifier {
  ...
  if ([peripheralArray count] <= 0) {
    NSLog(@"RFIDBluetoothManager: Can't find peripheral from identifier %@",
          identifier);
    return NO;
  }
...
// 設置標籤讀取格式
- (void)setEpcTidUserWithAddressStr:(NSString *)addressStr
                             length:(NSString *)lengthStr
                             epcStr:(NSString *)epcStr {
  self.isSetTag = YES;
  NSData *data = [BluetoothUtil setEpcTidUserWithAddressStr:addressStr
                                                     length:lengthStr
                                                     EPCStr:epcStr];
  // NSLog(@"data==%@",data);
  [self sendDataToBle:data];
...
// 設置發射功率
- (void)setLaunchPowerWithstatus:(NSString *)status
                         antenna:(NSString *)antenna
                         readStr:(NSString *)readStr
                        writeStr:(NSString *)writeStr {
  self.isSetEmissionPower = YES;
  NSData *data = [BluetoothUtil setLaunchPowerWithstatus:status
                                                 antenna:antenna
                                                 readStr:readStr
                                                writeStr:writeStr];
  [self sendDataToBle:data];
...
// 區域設置
- (void)setRegionWithsaveStr:(NSString *)saveStr
                   regionStr:(NSString *)regionStr {
  NSData *data = [BluetoothUtil setRegionWithsaveStr:saveStr
                                           regionStr:regionStr];
  [self sendDataToBle:data];
}
// 獲取區域設置
...
- (void)continuitySaveLabelWithCount:(NSString *)count {
  ...
  if (self.sendGetTagRequestTime == nil) {
    ...
    if (self.isBLE40 == YES) {
      self.sendGetTagRequestTime =
          [NSTimer timerWithTimeInterval:0.06
                                  target:self
    ...
    //       [runLoop addTimer:self.sendGetTagRequestTime
    //       forMode:NSRunLoopCommonModes]; [runLoop addPort:[NSMachPort port]
    //       forMode:NSRunLoopCommonModes]; [runLoop run];
    [[NSRunLoop mainRunLoop] addTimer:self.sendGetTagRequestTime
...
// 讀標籤數據區
- (void)readLabelMessageWithPassword:(NSString *)password
                              MMBstr:(NSString *)MMBstr
                              MSAstr:(NSString *)MSAstr
                              MDLstr:(NSString *)MDLstr
                              MDdata:(NSString *)MDdata
                               MBstr:(NSString *)MBstr
                               SAstr:(NSString *)SAstr
                               DLstr:(NSString *)DLstr
                            isfilter:(BOOL)isfilter {
  // NSLog(@"readLabelMessageWithPassword: %@, %@, %@, %@, %@, %@, %@, %@, %d",
  // password, MMBstr, MSAstr, MDLstr, MDdata, MBstr, SAstr, DLstr, isfilter);
  NSData *data = [BluetoothUtil readLabelMessageWithPassword:password
                                                      MMBstr:MMBstr
                                                      MSAstr:MSAstr
                                                      MDLstr:MDLstr
                                                      MDdata:MDdata
  ...
  for (int i = 0; i < [data length]; i += BLE_SEND_MAX_LEN) {
    ...
    if ((i + BLE_SEND_MAX_LEN) < [data length]) {
      NSString *rangeStr =
          [NSString stringWithFormat:@"%i,%i", i, BLE_SEND_MAX_LEN];
      ...
      NSString *rangeStr =
          [NSString stringWithFormat:@"%i,%i", i, (int)([data length] - i)];
      NSData *subData = [data subdataWithRange:NSRangeFromString(rangeStr)];
      [self sendDataToBle:subData];
      usleep(80 * 1000);
    }
...
// 寫標籤數據區
- (void)writeLabelMessageWithPassword:(NSString *)password
                               MMBstr:(NSString *)MMBstr
                               MSAstr:(NSString *)MSAstr
                               MDLstr:(NSString *)MDLstr
                               MDdata:(NSString *)MDdata
                                ...
                                SAstr:(NSString *)SAstr
                                DLstr:(NSString *)DLstr
                            writeData:(NSString *)writeData
                             isfilter:(BOOL)isfilter {
  NSData *data = [BluetoothUtil writeLabelMessageWithPassword:password
                                                       MMBstr:MMBstr
                                                       MSAstr:MSAstr
                                                       MDLstr:MDLstr
                                                       MDdata:MDdata
...
// Lock標籤
- (void)lockLabelWithPassword:(NSString *)password
                       MMBstr:(NSString *)MMBstr
                       MSAstr:(NSString *)MSAstr
                       MDLstr:(NSString *)MDLstr
                       MDdata:(NSString *)MDdata
                        ldStr:(NSString *)ldStr
                     isfilter:(BOOL)isfilter {
  NSData *data = [BluetoothUtil lockLabelWithPassword:password
                                               MMBstr:MMBstr
                                               MSAstr:MSAstr
                                               MDLstr:MDLstr
                                               MDdata:MDdata
...
// kill標籤。
- (void)killLabelWithPassword:(NSString *)password
                       MMBstr:(NSString *)MMBstr
                       MSAstr:(NSString *)MSAstr
                       MDLstr:(NSString *)MDLstr
                       MDdata:(NSString *)MDdata
                     isfilter:(BOOL)isfilter {
  NSData *data = [BluetoothUtil killLabelWithPassword:password
                                               MMBstr:MMBstr
                                               MSAstr:MSAstr
                                               MDLstr:MDLstr
                                               MDdata:MDdata
...
// 設置密鑰
- (void)setSM4PassWordWithmodel:(NSString *)model
                       password:(NSString *)password
                     originPass:(NSString *)originPass {
  NSData *data = [BluetoothUtil setSM4PassWordWithmodel:model
                                               password:password
...
// USER加密
- (void)encryptionUSERWithaddress:(NSString *)address
                        lengthStr:(NSString *)lengthStr
                          dataStr:(NSString *)dataStr {
  NSData *data = [BluetoothUtil encryptionUSERWithaddress:address
                                                lengthStr:lengthStr
                                                  dataStr:dataStr];
  // NSLog(@"data===%@",data);
  for (int i = 0; i < [data length]; i += BLE_SEND_MAX_LEN) {
...
// USER解密
- (void)decryptUSERWithaddress:(NSString *)address
                     lengthStr:(NSString *)lengthStr {
  NSData *data = [BluetoothUtil decryptUSERWithaddress:address
                                             lengthStr:lengthStr];
  [self sendDataToBle:data];
}
// 進入升級模式
...
- (void)sendtUpgradeDataWith:(NSData *)dataStr {
  ...
  for (int i = 0; i < [data length]; i += UpdateBLE_SEND_MAX_LEN) {
    ...
    if ((i + UpdateBLE_SEND_MAX_LEN) < [data length]) {
      NSString *rangeStr =
          [NSString stringWithFormat:@"%i,%i", i, UpdateBLE_SEND_MAX_LEN];
      NSData *subData = [data subdataWithRange:NSRangeFromString(rangeStr)];
      NSLog(@"%@", subData);
      [self sendDataToBle:subData];
      // 根據接收模塊的處理能力做相應延時
...
- (void)startBleScan {
  ...
  if (self.centralManager.state != CBManagerStatePoweredOn) {
    ...
    if ([self.managerDelegate
            respondsToSelector:@selector(connectBluetoothFailWithMessage:)]) {
      [self.managerDelegate
          connectBluetoothFailWithMessage:
              [self
                  centralManagerStateDescribe:CBCentralManagerStatePoweredOff]];
    }

    self.bleScanTimer = [NSTimer timerWithTimeInterval:0.5
                                                target:self
                                              selector:@selector(startBleScan)
                                              userInfo:nil
                                               repeats:NO];
    [[NSRunLoop mainRunLoop] addTimer:self.bleScanTimer
                              forMode:NSRunLoopCommonModes];
    NSLog(@"RFIDBluetoothManager: startBleScan: state is not powered on, retry "
          @"in 0.5 seconds");
    // Timeout of retrying
    if (!_connectTime || ![_connectTime isValid]) {
      _connectTime =
          [NSTimer timerWithTimeInterval:kFatscaleTimeOut
                                  target:self
                                selector:@selector(connectTimeroutEvent)
                                userInfo:nil
                                 repeats:NO];
      [[NSRunLoop mainRunLoop] addTimer:_connectTime
                                forMode:NSRunLoopCommonModes];
    }
    return;
  }
  ...
  //    {
  // 創建連接制定設備的定時器
  _connectTime = [NSTimer timerWithTimeInterval:kFatscaleTimeOut
                                         target:self
                                       selector:@selector(connectTimeroutEvent)
                                       userInfo:nil
                                        repeats:NO];
  ...
  NSArray *connectedPeripherals = [self.centralManager
  ...
  if (central.state != CBManagerStatePoweredOn) {
    if ([self.managerDelegate
      ...
      if (central.state == CBManagerStatePoweredOff) {
        ...
        [self.managerDelegate
            connectBluetoothFailWithMessage:
                [self centralManagerStateDescribe:CBManagerStatePoweredOff]];
      }
    }
  }
...
- (void)centralManager:(CBCentralManager *)central
    didDiscoverPeripheral:(CBPeripheral *)peripheral
        advertisementData:(NSDictionary<NSString *, id> *)advertisementData
                     RSSI:(NSNumber *)RSSI {
  NSData *manufacturerData = [advertisementData
      valueForKeyPath:CBAdvertisementDataManufacturerDataKey];
  ...
  //        NSData *subData = [manufacturerData
  //        subdataWithRange:NSMakeRange(manufacturerData.length-8, 8)];
...
- (void)getKeyData:(Byte *)data {
  ...
  for (int k = 0; k < dataKeyBuffLen; k++) {
    ...
    case 4:
      ...
      break;
    case 5:
      flag = (temp == 0x01 || temp == 0x02 || temp == 0x03 || temp == 0x04) ? 6
                                                                            : 0;
      keyCode = data[k];
      break;
    case 6:
...
- (void)parseKeyDown:(NSData *)data {
  ...
  for (int k = 0; k < data.length; k++) {
    ...
    if (dataIndex >= dataKeyBuffLen) {
      ...
      if (dataKey[0] == 0xC8 && dataKey[1] == 0x8c && dataKey[4] == 0xE6 &&
          dataKey[dataKeyBuffLen - 2] == 0x0D &&
          dataKey[dataKeyBuffLen - 1] == 0x0A) {
        [self getKeyData:dataKey];
      } else if (dataKey[0] == 0xA5 && dataKey[1] == 0x5A &&
                 dataKey[4] == 0xE6 && dataKey[dataKeyBuffLen - 2] == 0x0D &&
                 dataKey[dataKeyBuffLen - 1] == 0x0A) {
        [self getKeyData:dataKey];
      }
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  if (self.singleLableStr.length > 0) {
    ...
    if (dataStr.length < 40) {
      ...
      // c8 8c    00 19          81      34 00     39 31 31 31 32 32 32 32 33 33
  ...
  if (self.readStr.length > 0) {
    ...
    NSLog(@"RFIDBluetoothManager: [self.readStr appendString:dataStr];, "
          @"dataStr.length = %lu",
          (unsigned long)dataStr.length);
    if (dataStr.length < 40) {
      NSString *aa = [NSString stringWithFormat:@"%@", self.readStr];
      NSString *valueStr =
          [aa substringWithRange:NSMakeRange(18, aa.length - 18 - 6)];
  ...
  if (self.rcodeStr.length > 0) {
    ...
    //          NSData *parsedData = [self parseDataWithOriginalStr:rawData
    //          cmd:0xE5];
    //
    //          if (parsedData && parsedData.length > 0) {
    //               //NSLog(@"掃描二維碼111111111111111
    //               len=%d",parsedData.length);
    //               //NSLog(@"掃描二維碼=%@",[AppHelper dataToHex:parsedData]);
    //               Byte *bytes = (Byte *)parsedData.bytes;
    //               if (bytes[0] == 0x02) {
    //                    NSString *barcode = [[NSString
    //                    alloc]initWithData:parsedData
    ...
    //                         barcode = [[NSString
    //                         alloc]initWithData:parsedData encoding:enc];
    //                    }
    //
    //                    NSLog(@"掃描二維碼222=%@",barcode);
    //                    self.isCodeLab=NO;
    //                    self.rcodeStr=[[NSMutableString alloc]init];
    //                    [self.managerDelegate receiveMessageWithtype:@"e55"
    //                    dataStr:barcode];
    //               }
    //          }
    /*
    ...
    if (dataStr.length<40) {
         ...
         if (self.QRCodeSelType == selectedTypeOfDefault) {
              ...
              NSData *data = [self.rcodeStr
    dataUsingEncoding:NSUTF8StringEncoding]; aa = [[NSString
    alloc]initWithData:data encoding:NSUTF8StringEncoding]; } else if
    (self.QRCodeSelType == selectedTypeOfGB2312) { NSData *data = [self.rcodeStr
    dataUsingEncoding:NSUTF8StringEncoding]; NSStringEncoding enc =
    CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
              aa = [[NSString alloc] initWithData:data encoding:enc];
         }
         NSString *valueStr=[aa substringWithRange:NSMakeRange(12,
    aa.length-12-6)]; NSMutableString *strrr=[[NSMutableString alloc]init];

         char *myBuffer = (char *)malloc((int)[valueStr length] / 2 + 1);
             bzero(myBuffer, [valueStr length] / 2 + 1);
             for (int i = 0; i < [valueStr length] - 1; i += 2) {
             unsigned int anInt;
             NSString * hexCharStr = [valueStr substringWithRange:NSMakeRange(i,
    2)]; NSScanner * scanner = [[NSScanner alloc] initWithString:hexCharStr];
             [scanner scanHexInt:&anInt];
             myBuffer[i / 2] = (char)anInt;
             }
             NSString *unicodeString = [NSString stringWithCString:myBuffer
    encoding:4]; NSLog(@"------字符串=======%@",unicodeString);
  ...
  if (self.isgetLab == NO) {
    ...
    if ([typeStr isEqualToString:@"01"]) {
      ...
      if (self.isSetEmissionPower) {
        ...
        if ([strr isEqualToString:@"01"]) {
          [self.managerDelegate
              receiveMessageWithtype:@"11"
                             dataStr:@"Set power successfully"];
        } else {
          [self.managerDelegate receiveMessageWithtype:@"11"
                                               dataStr:@"Power setting fails"];
        }
      ...
      if (self.isGetEmissionPower) {
        // 獲取當前發射功率
        NSInteger a = [BluetoothUtil
            getzhengshuWith:[dataStr substringWithRange:NSMakeRange(14, 1)]];
        NSInteger b = [BluetoothUtil
            getzhengshuWith:[dataStr substringWithRange:NSMakeRange(15, 1)]];
        NSInteger c = [BluetoothUtil
            getzhengshuWith:[dataStr substringWithRange:NSMakeRange(16, 1)]];
        NSInteger d = [BluetoothUtil
            getzhengshuWith:[dataStr substringWithRange:NSMakeRange(17, 1)]];
        NSInteger count = (a * 16 * 16 * 16 + b * 16 * 16 + c * 16 + d) / 100;
        [self.managerDelegate
            receiveMessageWithtype:@"13"
                           dataStr:[NSString stringWithFormat:@"%ld", count]];
        self.isGetEmissionPower = YES;
      }
      ...
      if ([strr isEqualToString:@"01"]) {
        NSLog(@"跳頻設置成功");
        [self.managerDelegate
            receiveMessageWithtype:@"15"
                           dataStr:@"Set the frequency point successfully"];
      } else {
        NSLog(@"跳頻設置失敗");
        [self.managerDelegate
            receiveMessageWithtype:@"15"
                           dataStr:@"Failed to set frequency point"];
      }
      ...
      if (self.isRegion) {
        ...
        if ([strr isEqualToString:@"01"]) {
          ...
          [self.managerDelegate
              receiveMessageWithtype:@"2d"
                             dataStr:@"Set frequency successfully"];
        } else {
          [self.managerDelegate
              receiveMessageWithtype:@"2d"
                             dataStr:@"Failed to set frequency"];
        }
        self.isRegion = NO;
      }
      ...
      if ([strr isEqualToString:@"01"]) {
        ...
        [self.managerDelegate receiveMessageWithtype:@"2f"
                                             dataStr:@"讀取頻率失敗"];
      }
      ...
      // 讀標籤
      // NSLog(@"RFIDBluetoothManager: 85, dataStr.length = %lu", (unsigned
      // long)dataStr.length);
      if (dataStr.length < 40) {
        if (dataStr.length > 24) {
          NSString *strr = [dataStr
              substringWithRange:NSMakeRange(18, dataStr.length - 18 - 6)];
        ...
        if (dataStr.length == 40) {
          NSString *aa =
              [dataStr substringWithRange:NSMakeRange(dataStr.length - 4, 4)];
      ...
      NSString *strr = [dataStr substringWithRange:NSMakeRange(10, 2)];
      if ([strr isEqualToString:@"01"]) {
        [self.managerDelegate receiveMessageWithtype:@"87"
                                             dataStr:@"Successful tag writing"];
      } else {
        [self.managerDelegate receiveMessageWithtype:@"87"
                                             dataStr:@"Failed to write tag"];
      }
    } else if ([typeStr isEqualToString:@"89"]) {
      // lock標籤
      NSString *strr = [dataStr substringWithRange:NSMakeRange(10, 2)];
      if ([strr isEqualToString:@"01"]) {
        [self.managerDelegate receiveMessageWithtype:@"89"
                                             dataStr:@"Lock label successful"];
      } else {
        [self.managerDelegate receiveMessageWithtype:@"89"
                                             dataStr:@"Lock label failed"];
      }
    } else if ([typeStr isEqualToString:@"8b"]) {
      // 銷燬
      NSString *strr = [dataStr substringWithRange:NSMakeRange(10, 2)];
      if ([strr isEqualToString:@"01"]) {
        [self.managerDelegate receiveMessageWithtype:@"8b"
                                             dataStr:@"Destruction of success"];
      } else {
        [self.managerDelegate receiveMessageWithtype:@"8b"
      ...
      if (self.isSetTag) {
        ...
        NSString *strr = [dataStr substringWithRange:NSMakeRange(10, 2)];
        if ([strr isEqualToString:@"01"]) {
          [self.managerDelegate receiveMessageWithtype:@"71"
                                               dataStr:@"Successful setup"];
        }
      }
      ...
      if (self.isGetTag) {
        ...
        if ([strr isEqualToString:@"01"]) {
          ...
          NSString *addreStr = [BluetoothUtil
              becomeNumberWith:[dataStr substringWithRange:NSMakeRange(14, 2)]];
          NSString *addreLenStr = [BluetoothUtil
              becomeNumberWith:[dataStr substringWithRange:NSMakeRange(16, 2)]];
          NSString *allStr = [NSString
              stringWithFormat:@"%@ %@ %@", epcstr, addreStr, addreLenStr];
      ...
      if (self.isOpenBuzzer) {
        ...
        if ([strr isEqualToString:@"01"]) {
          [self.managerDelegate
              receiveMessageWithtype:@"e50"
                             dataStr:@"Buzzer turned on successfully"];
        }
      ...
      if (self.isCloseBuzzer) {
        ...
        if ([strr isEqualToString:@"01"]) {
          [self.managerDelegate
              receiveMessageWithtype:@"e51"
                             dataStr:@"Buzzer closed successfully"];
        }
      ...
      if (self.isGetBattery) {
        ...
        @try {
          ...
          NSInteger n = strtoul([battyStr UTF8String], 0,
                                16); // 16進制數據轉10進制的NSInteger
      ...
      if (self.isCodeLab) {
        ...
        //                    NSData *parsedData = [self
        //                    parseDataWithOriginalStr:rawData cmd:0xE5];
        //
        //                    if (parsedData && parsedData.length > 0) {
        //                         //NSLog(@"掃描二維碼111111111111111
        //                         len=%d",parsedData.length);
        //                         //NSLog(@"掃描二維碼=%@",[AppHelper
        //                         dataToHex:parsedData]); Byte *bytes = (Byte
        //                         *)parsedData.bytes; if (bytes[0] == 0x02) {
        //                              NSString *barcode = [[NSString
        //                              alloc]initWithData:parsedData
        //                              encoding:NSASCIIStringEncoding];
        //
        //                              if (self.QRCodeSelType ==
        //                              selectedTypeOfUTF8) {
        //                                   barcode = [[NSString
        //                                   alloc]initWithData:parsedData
        //                                   encoding:NSUTF8StringEncoding];
        //                              } else if (self.QRCodeSelType ==
        //                              selectedTypeOfGB2312) {
        //                                   NSStringEncoding enc =
        //                                   CFStringConvertEncodingToNSStringEncoding(kCFStringEncodingGB_18030_2000);
        //                                   barcode = [[NSString
        //                                   alloc]initWithData:parsedData
        ...
        //                              NSLog(@"掃描二維碼222=%@",barcode);
        //                              self.isCodeLab=NO;
        //                              self.rcodeStr=[[NSMutableString
        //                              alloc]init]; [self.managerDelegate
        //                              receiveMessageWithtype:@"e55"
        //                              dataStr:barcode];
        //                         }
        //                    }else {
        //                         self.rcodeStr=[[NSMutableString alloc]init];
        ...
        if (dataStr.length<40) {
             ...
             NSString *strr=[dataStr substringWithRange:NSMakeRange(12,
        dataStr.length-12-6)]; NSMutableString *strrr=[[NSMutableString
        alloc]init]; for(int i =1; i < [strr length]+1; i=i+2)
             {
        ...
        else
        {
             if (dataStr.length==40) {
                  NSString *aa=[dataStr
        substringWithRange:NSMakeRange(dataStr.length-4, 4)]; if ([aa
        isEqualToString:@"0d0a"]) {

                       NSString *strr=[dataStr
        substringWithRange:NSMakeRange(12, dataStr.length-12-6)];
                       NSMutableString *strrr=[[NSMutableString alloc]init];
                       for(int i =1; i < [strr length]+1; i=i+2)
                       {
                            NSString *aa=[strr substringWithRange:NSMakeRange(i,
        1)]; [strrr appendString:aa];
                       }
                       NSString *strrrr=[NSString stringWithFormat:@"%@",strrr];
                       [self.managerDelegate receiveMessageWithtype:@"e55"
        dataStr:strrrr]; self.isCodeLab=NO;
                  }
                  else
                  {
                       self.rcodeStr=[[NSMutableString alloc]init];
      ...
      //  setGen2
      [self
          parseSetGen2DataWithData:[NSData
                                       dataWithBytes:characteristic.value.bytes
                                              length:characteristic.value
                                                         .length]];
    } else if ([typeStr isEqualToString:@"23"]) {
      //  getGen2
      [self
          parseGetGen2DataWithData:[NSData
                                       dataWithBytes:characteristic.value.bytes
                                              length:characteristic.value
                                                         .length]];
    } else if ([typeStr.uppercaseString isEqualToString:@"6F"]) {
      //  setFilter
      [self parseFilterDataWithData:[NSData
                                        dataWithBytes:characteristic.value.bytes
                                               length:characteristic.value
                                                          .length]];
    } else if ([typeStr isEqualToString:@"53"]) {
      //  setRFLink
      [self parseSetRFLinkWithData:[NSData
                                       dataWithBytes:characteristic.value.bytes
                                              length:characteristic.value
                                                         .length]];
    } else if ([typeStr isEqualToString:@"55"]) {
      //  getRFLink
      [self parseGetRFLinkWithData:[NSData
                                       dataWithBytes:characteristic.value.bytes
                                              length:characteristic.value
                                                         .length]];
    }
    ...
    if (dataStr) {
      self.tagStr =
          (NSMutableString *)[self.tagStr stringByAppendingString:dataStr];
      if (!self.tagData) {
        self.tagData =
            [[NSMutableData alloc] initWithData:characteristic.value];
    ...
    //       數據長度（2字節）  cmd      pc(2字節)              epc rssi(2字節)
    //       ant((2字節))     crc(1字節)
    // c8 8c    00 19          e1      34 00     39 31 31 31 32 32 32 32 33 33
    // 33 34     fe b7      01               eb          0d 0a
    if (self.tagData.length > 0) {
      ...
      for (int s = 0; s < self.tagData.length; s++) {
        ...
        } else if ((tempBytes[index - 2] & 0xFF) == 0x0D &&
                   (tempBytes[index - 1] & 0xff) == 0x0A) {

          NSInteger len = ((tempBytes[2] & 0xFF) << 8) |
                          (tempBytes[3] & 0xFF); // 數據幀h長度

          NSData *tempNSData = [NSData dataWithBytes:tempBytes length:index];
          //  開始解析數據
          ////todo  NSLog(@"獲取到的正常數據爲：%@",[AppHelper
          ///dataToHex:tempNSData]);
          NSData *tagTempData = [self parseDataWithOriginalStr:tempNSData
                                                           cmd:0xE1];
...
- (void)parseReadTag_EPCWithDataStr:(NSData *)data {
  ...
  if (arr && arr.count) {
    ...
    [self.managerDelegate receiveDataWithBLEDataSource:arr
                                              allCount:self.allCount
                                              countArr:self.countArr
                                           dataSource1:self.dataSource1
                                             countArr1:self.countArr1
                                           dataSource2:self.dataSource2
                                             countArr2:self.countArr2];
  }
}
  ...
  for (NSInteger i = 0; i < count; i++) {
    ...
    for (NSInteger j = 0; j < self.dataSource.count; j++) {
      ...
      if ([oldEpcData isEqualToString:epcHex]) {
        ...
        [self.countArr
            replaceObjectAtIndex:j
                      withObject:[NSString
                                     stringWithFormat:@"%ld",
                                                      countStr.integerValue +
                                                          1]];
        break;
      }
    }
...
- (void)parseReadTagDataEPC_TID_USERWithData:(NSData *)tempData {
  ...
  for (NSInteger k = 0; k < count; k++) {
    ...
    epc_endIndex = epc_startIndex + (dataBytes[epc_lenIndex] &
                                     0xFF); // epc的起始索引加長度得到結束索引
    if (epc_endIndex > tempData.length) {
      ...
      [tempData
          getBytes:epcBuff
             range:NSMakeRange(epc_startIndex, epc_endIndex - epc_startIndex)];
      NSData *epcDataBuff =
          [NSData dataWithBytes:epcBuff length:epc_endIndex - epc_startIndex];
...
- (void)parserUhfTagBuff_EPC_TID_USER:(NSData *)tagsBuff {
  ...
  if (length >= uiiLen + 2 && epclen > 0) {
    Boolean isOnlyEPC =
        (length < (uiiLen + rssiLen + tidLen) ? YES : NO); // 只有epc
    Boolean isEPCAndTid =
        (length == (uiiLen + rssiLen + tidLen) ||
                 length == (uiiLen + rssiLen + tidLen + antLen)
             ? YES
             : NO); // 只有epc 和 tid
    Boolean isEPCAndTidUser = (length > (uiiLen + rssiLen + tidLen + antLen)
                                   ? YES
                                   : NO); // epc + tid + user
    BOOL isHave = NO;
    ...
    if (isEPCAndTidUser == YES) {
      ...
      //                //*************** EPC  and tid  user **************
      //               self.tagTypeStr = @"2";
      //               NSInteger userAndRssiLen=
      //               allData.length-(uiiLen*2+tidLen*2); userAndRssiLen=
      //               (userAndRssiLen%2!=0)? userAndRssiLen-1:
      //               userAndRssiLen;//有可能數據包含一個字節的天線號，所以這裏做特殊處理
      //               NSString * newUserAndRssiData= [allData
      //               substringWithRange:NSMakeRange(uiiLen*2+tidLen*2,userAndRssiLen)];
      //               NSString * newUserData= [newUserAndRssiData
      //               substringWithRange:NSMakeRange(0,newUserAndRssiData.length
      //               - rssiLen*2)]; isHave = NO; for (NSInteger j = 0 ; j <
      //               self.dataSource2.count; j ++) {
      //                    NSString *oldUserAndRssiData = self.dataSource2[j];
      //                    NSString *oldUser= [oldUserAndRssiData
      //                    substringWithRange:NSMakeRange(0,oldUserAndRssiData.length-rssiLen*2)];
      //
      //                    if ([newUserData isEqualToString:oldUser]) {
      //                         [self.dataSource2 replaceObjectAtIndex:j
      //                         withObject:newUserAndRssiData]; isHave = YES;
      //                         self.allCount ++;
      //                         NSString *countStr=self.countArr2[j];
      //                         [self.countArr2 replaceObjectAtIndex:j
      //                         withObject:[NSString
      //                         stringWithFormat:@"%ld",countStr.integerValue +
      //                         1]]; break;
      //                    }
      //               }
      //               if (!self.dataSource2 || self.dataSource2.count == 0 ||
      //               !isHave) {
      //                    [self.dataSource2 addObject:newUserAndRssiData];
      //                    [self.countArr2 addObject:@"1"];
      //                    NSString * EpcData= [allData
      //                    substringWithRange:NSMakeRange(4, epclen*2)];
      //                    [self.countArr addObject:@"1"];
      //                    [self.dataSource addObject:EpcData];
      //                    NSString * TidData= [allData
      //                    substringWithRange:NSMakeRange(uiiLen*2,tidLen*2 )];
      ...
      //               self.tagTypeStr = @"1";
      //               isHave = NO;
      //               NSString * newTidData= [allData
      //               substringWithRange:NSMakeRange(uiiLen*2,tidLen*2 )];
      //               NSString * tidAndRssiData= [allData
      //               substringWithRange:NSMakeRange(uiiLen*2,
      //               tidLen*2+rssiLen*2)]; for (NSInteger jTid = 0 ; jTid <
      //               self.dataSource1.count; jTid ++) {
      //                    NSString *oldTid = self.dataSource1[jTid];
      //                    oldTid= [oldTid
      //                    substringWithRange:NSMakeRange(0,oldTid.length-rssiLen*2)];
      //                    if ([oldTid isEqualToString:newTidData]) {
      //                         [self.dataSource1 replaceObjectAtIndex:jTid
      //                         withObject:tidAndRssiData]; isHave = YES;
      //                         self.allCount ++;
      //                         NSString *countStr=self.countArr1[jTid];
      //                         [self.countArr1 replaceObjectAtIndex:jTid
      //                         withObject:[NSString
      //                         stringWithFormat:@"%ld",countStr.integerValue +
      //                         1]]; break;
      //                    }
      //               }
      //               if (!self.dataSource1 || self.dataSource1.count == 0 ||
      //               !isHave) {
      ...
      //*************** EPC    **************
      NSString *newEpcData =
          [allData substringWithRange:NSMakeRange(4, epclen * 2)];
      NSString *epcAndRssiData =
          [allData substringWithRange:NSMakeRange(4, epclen * 2 + rssiLen * 2)];
      NSString *rssiStr =
          [epcAndRssiData substringFromIndex:epcAndRssiData.length - 4];
      NSInteger numOfRssiStr =
          [AppHelper getDecimalByBinary:[AppHelper getBinaryByHex:rssiStr]];
      ...
      //                     oldEPC= [oldEPC
      //                     substringWithRange:NSMakeRange(0,oldEPC.length-rssiLen*2
      //                     )];
      //                    if ([oldEPC isEqualToString:newEpcData]) {
      //                         isHave = YES;
      //                         self.allCount ++;
      //                         NSString *countStr=self.countArr[j];
      //                         [self.countArr replaceObjectAtIndex:j
      //                         withObject:[NSString
      //                         stringWithFormat:@"%ld",countStr.integerValue +
      //                         1]]; break;
      //                    }
    ...
    //          [self.managerDelegate
    //          receiveDataWithBLEDataSource:self.dataSource
    //          allCount:self.allCount countArr:self.countArr
    //          dataSource1:self.dataSource1 countArr1:self.countArr1
    //          dataSource2:self.dataSource2 countArr2:self.countArr2];
  }
}
...
- (NSString *)getVisiableIDUUID:(NSString *)peripheralIDUUID {
  ...
  peripheralIDUUID =
      [peripheralIDUUID stringByReplacingOccurrencesOfString:@"-"
                                                  withString:@""];
  peripheralIDUUID =
      [peripheralIDUUID stringByReplacingOccurrencesOfString:@"<"
                                                  withString:@""];
  ...
  peripheralIDUUID =
      [peripheralIDUUID stringByReplacingOccurrencesOfString:@" "
                                                  withString:@""];
  peripheralIDUUID =
      [peripheralIDUUID substringFromIndex:peripheralIDUUID.length - 12];
...
- (void)setGen2WithTarget:(char)Target
                   action:(char)Action
                        t:(char)T
                       qq:(char)Q_Q
                   startQ:(char)StartQ
  ...
  NSData *byteData = [self setGen2DataWithTarget:Target
                                          action:Action
                                               t:T
                                              qq:Q_Q
                                          startQ:StartQ
...
- (NSData *)setGen2DataWithTarget:(char)Target
                           action:(char)Action
                                t:(char)T
                               qq:(char)Q_Q
                           startQ:(char)StartQ
                          ...
                          session:(char)Session
                               ...
                               lf:(char)LF {
  Byte sbuf[4];
  sbuf[0] = (((Target & 0x07) << 5) | ((Action & 0x07) << 2) |
             ((T & 0x01) << 1) | ((Q_Q & 0x01) << 0));
  sbuf[1] = (((StartQ & 0x0f) << 4) | ((MinQ & 0x0f) << 0));
  sbuf[2] = (((MaxQ & 0x0f) << 4) | ((D_D & 0x01) << 3) | ((C_C & 0x03) << 1) |
             ((P_P & 0x01) << 0));
  sbuf[3] = (((Sel & 0x03) << 6) | ((Session & 0x03) << 4) |
             ((G_G & 0x01) << 3) | ((LF & 0x07) << 0));
  // return sbuf;
...
- (void)getGen2SendData {
  ...
  Byte sbuf[0];
  NSData *bytesData = [self makeSendDataWithCmd:0x22
                                        dataBuf:[NSData dataWithBytes:sbuf
                                                               length:0]];
...
- (void)parseGetGen2DataWithData:(NSData *)data {
  ...
  if (self.managerDelegate &&
      [self.managerDelegate
          respondsToSelector:@selector(receiveGetGen2WithData:)]) {
...
- (void)parseSetGen2DataWithData:(NSData *)data {
  ...
  if (self.managerDelegate &&
      [self.managerDelegate
          respondsToSelector:@selector(receiveSetGen2WithResult:)]) {
    [self.managerDelegate receiveSetGen2WithResult:parseResult];
  }
}

//----------------------------------------設置Filter--------------------------------------------------------------------------------------
- (void)setFilterWithBank:(int)bank
                      ptr:(int)ptr
                      cnt:(int)cnt
                     data:(NSString *)data {
...
- (void)parseFilterDataWithData:(NSData *)data {
  ...
  if (self.managerDelegate &&
      [self.managerDelegate
          respondsToSelector:@selector(receiveSetFilterWithResult:)]) {
...
- (void)setRFLinkWithMode:(int)mode {
  ...
  NSData *sendRFLinkData = [self makeSendDataWithCmd:0x52
                                             dataBuf:rfLinkSetData];
...
- (void)parseSetRFLinkWithData:(NSData *)data {
  ...
  if (self.managerDelegate &&
      [self.managerDelegate
          respondsToSelector:@selector(receiveSetRFLinkWithResult:)]) {
...
- (void)parseGetRFLinkWithData:(NSData *)data {
  ...
  if (self.managerDelegate &&
      [self.managerDelegate
          respondsToSelector:@selector(receiveGetRFLinkWithData:)]) {
...
- (void)parseSingleLabel:(NSData *)tagTempData {
  ...
  if (tagTempData && tagTempData.length >= 4) {
    ...
    int antLen = 1;
    NSInteger userLen =
        tagTempData.length - pcLen - epcLen - tidLen - rssiLen - antLen;
    userLen =
        (userLen % 2 != 0)
            ? userLen - 1
            : userLen; // 有可能數據包含一個字節的天線號，所以這裏做特殊處理
    ...
    if (self.isSupportRssi == YES) {
      if (tagTempData.length > pcLen + epcLen + rssiLen + antLen + tidLen) {
        ...
        self.tagTypeStr = @"2";
        NSString *realEPCStr =
            [hexData substringWithRange:NSMakeRange(4, epcLen * 2)];
        NSString *TidStr = [hexData
            substringWithRange:NSMakeRange(4 + epcLen * 2, tidLen * 2)];
        NSString *userAndRSSIStr =
            [hexData substringWithRange:NSMakeRange(4 + epcLen * 2 + tidLen * 2,
                                                    userLen * 2 + rssiLen * 2)];
        NSString *newUserData = [userAndRSSIStr
            substringWithRange:NSMakeRange(0, userAndRSSIStr.length -
                                                  rssiLen * 2)];
        BOOL isHave = NO;
        for (NSInteger j = 0; j < self.dataSource2.count; j++) {
          NSString *oldUserAndRssiData = self.dataSource2[j];
          NSString *oldUser = [oldUserAndRssiData
              substringWithRange:NSMakeRange(0, oldUserAndRssiData.length -
                                                    rssiLen * 2)];
          if ([newUserData isEqualToString:oldUser]) {
        ...
        NSString *realEPCStr =
            [hexData substringWithRange:NSMakeRange(4, epcLen * 2)];
        NSString *newTidData = [hexData
            substringWithRange:NSMakeRange(4 + epcLen * 2, tidLen * 2)];
        NSString *tidAndRssiData =
            [hexData substringWithRange:NSMakeRange(4 + epcLen * 2,
                                                    tidLen * 2 + rssiLen * 2)];
        for (NSInteger jTid = 0; jTid < self.dataSource1.count; jTid++) {
          ...
          if ([oldTid isEqualToString:newTidData]) {
            [self.dataSource1 replaceObjectAtIndex:jTid
                                        withObject:tidAndRssiData];
            isHave = YES;
            self.allCount++;
            NSString *countStr = self.countArr1[jTid];
            [self.countArr1
                replaceObjectAtIndex:jTid
                          withObject:[NSString stringWithFormat:
                                                   @"%ld",
                                                   countStr.integerValue + 1]];
            [self.countArr
        ...
        NSString *realEPCStr =
            [hexData substringWithRange:NSMakeRange(4, epcLen * 2)];
        NSString *realEPCAndRssi = [hexData
            substringWithRange:NSMakeRange(4, epcLen * 2 + rssiLen * 2)];
        BOOL isHave = NO;
        for (NSInteger j = 0; j < self.dataSource.count; j++) {
          NSString *oldEPC = self.dataSource[j];
          oldEPC = [oldEPC
    ...
    [self.managerDelegate receiveDataWithBLEDataSource:self.dataSource
                                              allCount:self.allCount
                                              countArr:self.countArr
                                           dataSource1:self.dataSource1
                                             countArr1:self.countArr1
...
- (void)initSoundIfNeeded {
  if (self->soundInitialized)
    return;

  NSLog(@"RFIDBluetoothManager: initSound");
  NSString *soundFile1 = [[NSBundle mainBundle] pathForResource:@"beep"
                                                         ofType:@"mp3"];
  NSError *error1;
  for (player1i = 0; player1i < SOUND_I; player1i++) {
    self->player1[player1i] = [[AVAudioPlayer alloc]
        initWithContentsOfURL:[NSURL fileURLWithPath:soundFile1]
                        error:&error1];
    self->player1[player1i].numberOfLoops = 1;
  }

  NSString *soundFile2 = [[NSBundle mainBundle] pathForResource:@"beep_slight"
                                                         ofType:@"mp3"];
  NSError *error2;
  for (player2i = 0; player2i < SOUND_I; player2i++) {
    self->player2[player2i] = [[AVAudioPlayer alloc]
        initWithContentsOfURL:[NSURL fileURLWithPath:soundFile2]
                        error:&error2];
    self->player2[player2i].numberOfLoops = 1;
  }

  NSString *soundFile3 = [[NSBundle mainBundle] pathForResource:@"serror"
                                                         ofType:@"mp3"];
  NSError *error3;
  for (player3i = 0; player3i < SOUND_I; player3i++) {
    self->player3[player3i] = [[AVAudioPlayer alloc]
        initWithContentsOfURL:[NSURL fileURLWithPath:soundFile3]
                        error:&error3];
...
- (void)initHapticFeedbackIfNeeded {
  if (self->hapticFeedbackInitialized)
    return;

  self->uiLightImpactFeedbackGenerator = [[UIImpactFeedbackGenerator alloc]
...
- (void)playTagScannedFeedback:(int)soundId
                        volume:(float)volume
       hapticFeedbackIntensity:(float)hapticFeedbackIntensity {
  ...
  if (!lastTagScannedFeedbackPlayAt ||
      currentTime - lastTagScannedFeedbackPlayAt >= self.feedbackMinimumDelay) {
    if (volume > 0) {
      [self playSound:soundId withVolume:volume];
    }
    if (hapticFeedbackIntensity > 0) {
      [self->uiLightImpactFeedbackGenerator
          impactOccurredWithIntensity:hapticFeedbackIntensity];
    }
    if (soundId == 1) {
    ...
    double delayInMs = self.feedbackMinimumDelay -
                       (currentTime - lastTagScannedFeedbackPlayAt);
    if (delayInMs > 400) {
      if (soundId == 1) {
        delayInMs = 400;
      } else {
    ...
    dispatch_after(
        dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInMs * NSEC_PER_MSEC)),
