...
const jsonSchemaFilePath = path.join(__dirname, '..', 'schema.json');
const outputFilePath = path.join(__dirname, '..', 'generated-schema.ts');
...
function copyAppFile(filePath: Array<string>) {
  const srcPath = path.join(
    ...[__dirname, '..', '..', 'App', 'app', ...filePath],
  );
  const toPath = path.join(...[__dirname, '..', 'vendor', 'app', ...filePath]);
...
copyAppFile(['modules', 'EPCUtils.ts']);
copyAppFile(['lib', 'types', 'epc-tds.d.ts']);
