...
export async function collectionToAirtableRecord(
  ...
  const fields = {
    ...
    ID: collection.__id,
    'Ref. Number': collection.collection_reference_number,
  };
...
export async function airtableRecordToCollection(
  ...
  if (typeof record.fields['Ref. Number'] === 'string') {
    collection.collection_reference_number = record.fields['Ref. Number'];
  }
...
export async function airtableRecordToItem(
  ...
  if (Array.isArray(record.fields.Container)) {
    ...
    if (containerRecordId) {
      // TODO: use cache
      const containers = await getData('item', {
        integrations: { [integrationId]: { id: containerRecordId } },
      });
      item.container_id = containers[0]?.__id;
    } else {
