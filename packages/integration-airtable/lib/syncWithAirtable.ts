...
import { DataType } from '@deps/data/schema';
import {
  DataMeta,
  GetData,
  GetDataConditions,
...
async function filterArrayWithAsyncFunction<T extends ReadonlyArray<unknown>>(
  array: T,
  fn: (item: T[number], index: number) => Promise<boolean>,
): Promise<T> {
  const filterFlags = await Promise.all(array.map(fn));
  return array.filter((_, index) => filterFlags[index]) as any;
}
...
export default async function* syncWithAirtable(
  ...
  if (fullSync) {
    try {
      ...
      if (e instanceof Error) {
        e.message = 'Error on fullSync listRecords - Collections: ' + e.message;
      }
  ...
  async function* syncData<T extends 'collection' | 'item'>(
    ...
    try {
      ...
      console.log(`TODO ${type} to create: ${dataToCreate.length}`);
    ...
    try {
      ...
      while (true) {
        ...
        for (const record of resp.records) {
          if (recordIdsToDelete.has(record.id)) continue;
  ...
  for await (const p of syncData(
    ...
    'Collections',
    {
      ...
      airtableFields: ['Name', 'Ref. Number'],
    },
  )) {
    yield p;
  }
  ...
  async function getAirtableRecordIdFromItemId(itemId: string) {
    ...
    if (
      ...
      console.log('Creating item - TODO 28399832');
      if (!item.__valid) {
  ...
  for await (const p of syncData(
    ...
    'Items',
    {
      ...
      airtableRecordToDatum: async r =>
        airtableRecordToItem(r, { integrationId, getData }),
  ...
  // const itemsToSync = await getData('item', {
  //   collection_id: { $in: collection_ids_to_sync } as any,
  // });
  // const itemsToCreate = onlyValid(
  //   itemsToSync.filter(
  //     c =>
  //       !c.integrations ||
  //       !(c.integrations as any)[integrationId] ||
  //       !(c.integrations as any)[integrationId].id ||
  //       (fullSync &&
  //         !fullSync_existingItems.find(
  //           r => r.id === (c.integrations as any)[integrationId].id,
  //         )),
  //   ),
  // );
  // progress.toPush = (progress.toPush || 0) + itemsToCreate.length;

  // yield progress;

  // for (let i = 0; i < itemsToCreate.length; i += 10) {
  //   const itemsChunk = itemsToCreate.slice(i, i + 10);
  //   const records = await executeSequentially(
  //     itemsChunk.map(
  //       it => () =>
  //         itemToAirtableRecord(it, {
  //           airtableItemsTableFields,
  ...
  //     ),
  //   );
  //   const results = await api.createRecords(config.airtable_base_id, 'Items', {
  //     records: records.filter(r => !createdItemIds.has(r.fields.ID || '')),
  //   });
  ...
  //   progress.pushed = (progress.pushed || 0) + itemsChunk.length;
